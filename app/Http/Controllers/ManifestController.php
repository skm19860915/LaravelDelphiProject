...
class ManifestController extends Controller
{
    ...
    {
        $xerequests = DB::select('SELECT os.id, os.date_created, os.remarks, os.client_id, os.account_no AS cno, os.name AS cname,
        ...
        return view('manifest.index', ['xerequests' => $xerequests]);
    }

    public function filter(Request $request)
    {
        $request->session()->put('ids', $request->all());
    ...
    public function export()
    {
        ...
        $templateProcessor = new TemplateProcessor('word-template/manifest.docx');
        $templateProcessor->setValue('id', 'UC'.$ids[0]);
        $templateProcessor->saveAs('MANIFEST.doc');
        return response()->download('MANIFEST.doc')->deleteFileAfterSend(true);
    }
}
